test_name "PUP-5867: The report specifies whether a cached catalog was used, and if so, why" do
  testdir = create_tmpdir_for_user master, 'reportdir'

  teardown do
    on(master, "rm -rf #{testdir}")
  end

  def remove_reports_on_master(testdir, agent_node_name)
    on(master, "rm -rf #{testdir}/#{agent_node_name}/*")
  end
