...
Puppet::Type.type(:package).provide :rpm, :source => :rpm, :parent => Puppet::Provider::Package do
  ...
  # named for hit musical group of the 1990s * N S Y N C
	def nsync(is)
    return false if [:purged, :absent].include?(is)
		should = resource[:ensure]
    0 == yum_compareEVR(yum_parse_evr(should), yum_parse_evr(is))
	end
  ...
  def self.nevra_to_hash(line)
    ...
    if match = self::NEVRA_REGEX.match(line)
      ...
      hash[:ensure] = "#{hash[:version]}-#{hash[:release]}"
			hash[:ensure].prepend("#{hash[:epoch]}:")
			hash[:ensure].concat(".#{hash[:arch]}")
    else
  ...
  def yum_compareEVR(should_hash, is_hash)
    ...
    rc = compare_values(should_hash[:release], is_hash[:release])
		if rc != 0
			is_release = is_hash[:release].split('.')[0..-2].join('.')
			rc = compare_values(should_hash[:release], is_release)
		end
